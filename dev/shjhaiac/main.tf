module "base" {
  ...
  lnet-endingAddress   = "192.168.1.190"  # This IP range should not overlap with HCI infra IP range.
  lnet-addressPrefix   = "192.168.1.0/24"  # E.g., 192.168.1.0/24
  lnet-defaultGateway  = "192.168.1.1" # Default gateway can be same as HCI infra default gateway.
  lnet-dnsServers      = ["192.168.1.254"]   # DNS servers can be same as HCI infra DNS servers.

  # Region AKS Arc parameters
  aksArc-controlPlaneIp   = "192.168.1.190"      # An IP address in the logical network IP range.
  rbacAdminGroupObjectIds = ["2a429c24-2aed-455b-974e-6d6710da8f7e"] # An AAD group that will have the admin permission of this AKS Arc cluster. Check ./doc/AKS-Arc-Admin-Groups.md for details
